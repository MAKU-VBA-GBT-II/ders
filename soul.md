...
| H3–4 | **Görev 1** — Mini Veri Hattı (tüm şirketlerde aynı küçük uygulama) | `gorev1-final` |
...
Her şirkette **proje lideri dönem boyu PM'dir** — dönem başında iş ilanıyla seçilir (§1.4). Kalan 4 üye soyadına göre alfabetik olarak **P2–P5** diye numaralanır; BE, FE, DA ve QA rolleri her küçük görevde aralarında döner. Böylece herkes büyük projede sabit rol seçmeden önce 3 farklı rolü deneyimlemiş olur; lider ise üç görev boyunca PM tecrübesi biriktirir.

| Pozisyon | Görev 1 (H3–4) | Görev 2 (H5–6) | Görev 3 (H7–8) | Büyük Proje (H9–15) |
|---|---|---|---|---|
| **Lider** | PM | PM | PM | PM (sabit) |
| P2 | BE | FE | DA | Hafta 9'da müzakere edilir |
| P3 | FE | DA | QA | Hafta 9'da müzakere edilir |
| P4 | DA | QA | BE | Hafta 9'da müzakere edilir |
| P5 | QA | BE | FE | Hafta 9'da müzakere edilir |
...
Roleplay'in ilk adımı bir **işe alım sürecidir**: dönem başında her şirket için bir "proje lideri" ilanı açılır, öğrenciler başvurur, öğretim elemanı 6 lideri seçer.

1. **İlan (`<İLAN TARİHİ>`):** Öğretim elemanı 6 ilanı yayımlar — her biri bir sektöre bağlıdır (§1.1 tablosu). İlan metninde: şirketin sektörü, liderin görev tanımı (§1.2 PM satırı), aranan nitelikler (birinci dönem temellerine hâkimiyet, düzenli çalışma alışkanlığı, ekip koordinasyonu isteği).
2. **Başvuru (`<BAŞVURU SON TARİHİ>`'a kadar):** Adaylar, öğretim elemanının paylaştığı **Google Form** üzerinden başvurur. Form alanları:
   - Ad-soyad, öğrenci no.
   - Tercih sırası: ilgilendiği sektörler (1'den 6'ya sıralama).
...
3. **Seçim (`<SEÇİM GÜNÜ>`):** Öğretim elemanı başvuruları değerlendirir — gerekçenin açıklığı, birinci dönem izleri, sektör tercihleri. 6 lider ilan edilir; sonuçlar `README.md`'lere işlenir.
4. **Liderin ilk görevleri (Hafta 1–2):** Şirket adını ekibe önerip oylatmak, teknoloji kararına öncülük etmek, Hafta 2'de panoyu kurmak.
5. **Seçilemeyen adaylar:** P2–P5 numaralarıyla rotasyona katılırlar — bu bir kayıp değildir; rotasyon herkesin 3 rol denemesini garanti eder ve büyük projede rol seçiminde tecrübe avantajı sağlar.
...
**Nasıl açılır:** Depo → `Issues` → `New issue` → **Başlık** (örn. `G1-veri — Simülasyonu yaz`) → **Açıklama**: ne yapılacak + tek cümlelik kabul kriteri + son tarih → sağ panelden **Assignees** (yapacak kişi) ve **Labels** (`bug`/`enhancement`) → `Submit new issue`.

**Nasıl kapatılır:** İş bitince Issue'nun altına kanıt içeren yorum yazın (örn. "Tamamlandı — commit `a3f9c21`, çıktı: `data/sales.csv` 120 satır") → `Close issue`. Kanıtsız kapatılan Issue yapılmamış sayılır.
...
7. **İlk kişisel commit:** `README.md`'yi açın, listeye kendi satırınızı ekleyin: lider seçildiyse `- Ad Soyad — PM (lider)`, diğer üyeler `- Ad Soyad — P2` … `P5` biçiminde (soyadı alfabetik sırasına göre). Kaydedin → commit (mesaj: `README'ye Ad Soyad eklendi`) → `Push origin`. Bu commit, "bu öğrenci araç zincirini kurdu" kanıtıdır.
...
- [ ] 5 üyenin 5'i de commit geçmişinde görünüyor.
...
### GÖREV 1 — "Mini Veri Hattı" (Hafta 3–4)

**Ürün (tüm şirketler için aynı):** Küçük bir veri hattı: bir kahve dükkanı satış **simülasyonu** veri üretir, bir analiz bu veriden 3 metrik çıkarır, bir görselleştirme 2 grafik üretir, bir test süreci veri kalitesini sınar.
...
1. 4 Issue açın. Her Issue'nun açıklamasında **son tarih** ve **tek cümlelik kabul kriteri** bulunmak zorundadır:
   ...
   |---|---|---|
   | `G1-veri — Satış simülasyonunu yaz` | BE | `data/sales.csv` depoda: 30 gün × 4 ürün = 120 satır, şeması commit mesajında veya yorumda yazılı. |
   | `G1-analiz — 3 metriği hesapla` | DA | `docs/analiz-g1.md` depoda: toplam gelir, en çok satan ürün, günlük ortalama adet hesaplanmış ve yorumlanmış. |
   | `G1-gorsel — 2 grafik üret` | FE | `visuals/gunluk-satis.png` ve `visuals/urun-pay.png` depoda; grafikler BE'nin verisinden üretilmiş. |
...
**BE (P2) şu adımları uygular:**

1. Satış simülasyonu betiğini `/src` içine yazın: 30 gün boyunca 4 ürün (Latte, Espresso, Cappuccino, Filtre) için günlük satış adedi üretin; her ürünün sabit birim fiyatı olsun (örn. 40/30/45/25 TL).
2. `data/sales.csv` dosyasını üretin. **Zorunlu sütunlar:** `tarih (yyyy-mm-dd)`, `urun (metin)`, `adet (0–50 tam sayı)`, `birim_fiyat (sayı)`. **Zorunlu boyut:** 120 satır + başlık satırı.
3. Commit mesajına **şemayı** yazın: *"sales.csv — 120 satır; sütunlar: tarih, urun, adet(0-50), birim_fiyat"*. (Kural §3.4: şemasız veri teslim edilmemiş sayılır.)
4. **Devir teslim yorumu (notlandırılır):** `G1-analiz` ve `G1-gorsel` Issue'larının her birine şu yorumu yazın: *"Veri hazır — commit `<hash>`. Dosya: `data/sales.csv` (120 satır). Şema: tarih, urun, adet(0-50), birim_fiyat."* Bu yorum, "BE işini teslim etti, DA ve FE başlayabilir" anlamına gelen resmî devir teslimdir.

**DA (P4) şu adımları uygular:**

1. BE'nin devir teslim yorumunu bekleyin; veri hazır değilken analiz yazmayın (zincirin amacı bu).
2. `docs/analiz-g1.md` dosyasını yazın. İçerik zorunlu:
   - **3 metrik:** toplam gelir (adet × fiyat toplamı), en çok satan ürün (adet bazında), günlük ortalama satış adedi (4 ürün toplamı / 30).
   - Her metriğin altında **1-2 cümle yorum** ("Hafta sonları düşüyor mu? Hangi ürün baskın?").
   - Hangi veri dosyasından, hangi commit'ten çalıştığınızı yazın (iz sürülebilirlik).
3. Analiz kodunu `/src` içine kaydedin (bir betik, notebook veya doğrudan belge içi hesaplama — tercih serbest ama tekrarlanabilir olmalı).
4. **Devir teslim yorumu (notlandırılır):** `G1-gorsel` Issue'suna yazın: *"Analiz hazır — `docs/analiz-g1.md`, commit `<hash>`. 3 metrik + yorumlar. FE görselleştirebilir."*

**FE (P3) şu adımları uygular:**

1. Grafikleri **yapmadan önce** kâğıda veya ekrana taslağını çizin: iki grafik nasıl görünecek, eksenler ne olacak, başlıklar ne diyecek.
2. Taslağın fotoğrafını/ekran görüntüsünü `G1-gorsel` Issue'suna yorum olarak ekleyin (görseli yorum kutusuna sürükleyip bırakmak yeterli). **Taslak eklenmeden grafik yapımına başlamak kural ihlalidir.**
3. İki grafiği üretin ve PNG olarak `/visuals` klasörüne kaydedin:
   - `gunluk-satis.png`: günlük toplam satış adedi çizgi grafiği (30 nokta).
   - `urun-pay.png`: ürünlerin toplam satış payı pasta grafiği.
4. BE ve DA'nın devir teslim yorumlarını bekleyin; grafikler **BE'nin verisinden** üretilmeli. Grafik üretim kodunu `/src` içine kaydedin.
5. Commit + push; Issue'ya "grafikler hazır — commit `<hash>`, çıktılar: `visuals/gunluk-satis.png`, `visuals/urun-pay.png`" yorumu yazın.

**QA (P5) şu adımları uygular:**

1. `tests/test-cases.md` dosyasını şablona (§7.3) göre yazın; en az 5 senaryo:
   - Dosya var mı ve boş değil mi?
   - Satır sayısı 120 mi?
   - Sütun adları beklenenlerle aynı mı (`tarih, urun, adet, birim_fiyat`)?
   - `adet` değerleri 0–50 aralığında mı ve negatif var mı?
   - `urun` sütununda beklenen 4 üründen başka değer var mı?
2. Her senaryoda şunlar yazılı olmalı: ID (T1, T2…), adımlar, girdi, beklenen sonuç. "Gerçekleşen sonuç" ve "geçti/kaldı" sütunları Hafta 4'te doldurulacak.
3. Commit + push; `G1-test` Issue'suna kanıt yorumu yazıp kapatın.

#### Hafta 4

**QA:**
1. Tüm test senaryolarını gerçek `data/sales.csv` üzerinde tek tek çalıştırın; sonuçları `tests/test-log.md` dosyasına tarihli olarak işleyin (gerçekleşen sonuç + geçti/kaldı).
2. **En az 2 hata Issue'su açın**, `bug` etiketiyle. Şablon: *Başlık:* belirtiyi özetler (örn. `sales.csv'de negatif adet değeri var`). *Açıklama:* yeniden üretme adımları, beklenen davranış, gözlenen davranış, varsa ekran görüntüsü. Gerçekten hiç hata bulamazsanız `enhancement` etiketiyle 2 iyileştirme önerisi açın (örn. "Simülasyona hafta sonu etkisi eklensin").
...
**DA:** Nihai `data/sales.csv` dosyasının BE'nin ilan ettiği şemayla uyumlu olduğunu doğrulayın; `G1-analiz` Issue'suna kapanış yorumu yazın: *"Analiz nihai veri üzerinde doğrulandı — onaylıyorum."* Uyumsuzluk varsa farkları listeleyip BE'ye atayın.
...
- *Grup eşikleri:* `gorev1-final` release'i son tarihte mevcut · devir teslim zinciri kanıtlı (BE'nin `G1-analiz`/`G1-gorsel`'e veri yorumu + DA'nın `G1-gorsel`'e analiz yorumu; BE yorumu DA yorumundan önce tarihli) · `bug` etiketli ≥2 Issue açılmış ve `Fixes #n` ile kapatılmış · `report-w03.md` ve `report-w04.md` zamanında commit'lenmiş.
...
**Ürün:** Şirketiniz veri kümesini **dışa aktarır** (export); partner şirket bu dosyayı **içe aktarıp analiz eder ve görselleştirir** (import). Eşleşmeler: A↔B, C↔D, E↔F (§1.1). Her çift, ortak bir **veri sözleşmesi** müzakere eder. Roller yine döner (§1.3).

**Sözleşme nedir:** İki tarafın da uyacağı yazılı dosya formatı tanımıdır — gerçek sektörde iki şirketin API/veri anlaşmasının küçük ölçekli hali. **Veri değişiminin kendisi notlandırılır:** kim neyi, hangi formatta gönderdi; karşı taraf bunu hatasız okuyabildi mi; sözleşmeye uyuldu mu.

**Zemin:** Görev 3, Görev 2'deki mini ürünün üzerine inşa edilir — aynı depoyla, aynı sektör verisiyle devam edersiniz.

**Dosya değişimi GitHub üzerinden yapılır:** Her şirket, dışa aktardığı dosyayı kendi deposunda `exchange/output.csv` yoluna commit'ler; partner şirket dosyayı oradan alır (depolar herkese açık; indirmek için dosyanın GitHub sayfası → `Raw` → sağ tık → farklı kaydet). **Ara teslim:** `exchange/output.csv` en geç **Hafta 8'de, `<ARA TESLİM GÜNÜ/SAATİ>`'ne kadar** depoda olmalı — yoksa partnerin içe aktarmayı test edecek zamanı kalmaz.

#### Hafta 7 — Sözleşme haftası

**DA (P2) şu adımları uygular:**

1. Partner şirketin DA'sıyla sözleşmeyi müzakere edin — **sözlü değil**, GitHub üzerinde: her iki depoda da `Veri sözleşmesi A↔B` (kendi eşleşmenize göre) başlıklı birer Issue açın; müzakere bu Issue'ların yorumlarında yürür (kim neyi önerdi, neye karar verildi — iz bırakır).
2. Şunlarda anlaşın: dosya adı ve format (CSV veya JSON), alan adları ve sırası, tipler, ayraç (CSV ise `;` mı `,` mı), kodlama (UTF-8 önerilir), tarih formatı (`yyyy-mm-dd` önerilir), ondalık ayracı, başlık satırı olup olmayacağı, birimler (TL, km, adet…), hatalı satır kuralı.
3. Anlaşmayı `docs/veri-sozlesmesi.md` olarak yazın (şablon §7.5'te) ve **her iki depoya da birebir aynı** commit'leyin. İki dosya arasında tek karakter fark olması sözleşme ihlalidir.

**BE (P4) şu adımları uygular:**
...
**FE (P5):** Görev 2 çıktılarına bir "dışa aktarım" dokunusu ekleyin: tek komutla/betikle `exchange/output.csv` üretilsin (örneğin `src/export.py` — çalıştırınca dosyayı üretsin; dilinizin imkanlarına göre parametre de alabilir). Branch + PR + merge.

**QA (P3):** Çapraz testleri tasarlayıp `tests/test-cases.md`'ye ekleyin: eksik sütunlu dosya gelirse ne olmalı? Yanlış tarih formatı? Tamamen boş dosya? Fazladan sütun? Her senaryo için beklenen davranışı yazın ("program çöker" hiçbir senaryoda kabul edilebilir davranış değildir).
...
2. Partnerin deposundaki `exchange/output.csv` dosyasını indirin; onu okuyup işleyen `ImportCSV` betiğini yazın: sözleşmedeki alan adlarını ve tiplerini doğrulayarak `data/partner-verisi.csv` olarak (veya doğrudan belleğe) yükleyin. Kendi ürettiğiniz test dosyasıyla değil, **partnerin gerçek çıktısıyla** test edin — bütün mesele bu.
...
- *Grup eşikleri:* `gorev3-final` release'i son tarihte mevcut · `exchange/output.csv` `<ARA TESLİM GÜNÜ/SAATİ>` ara teslimine kadar depodaydı · iki depoda birebir aynı sözleşme dosyası · partnerin gerçek dosyasının başarıyla içe aktarılması (hoca derste canlı test eder) · partner deposunda "Değişim sonucu" Issue'su açılmış, dili profesyonel ve somut.
...
| urun | Metin | 4 sabit değerden biri | Latte |
...
|---|---|---|---|---|---|---|
| T1 | G1 | sales.csv'yi aç, satır say | — | 120 veri satırı | | |
